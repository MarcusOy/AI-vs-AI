...
public class API {
	...
	public String[] getValidMoves(int col, int row, int myColor, String[][] board) {
		...
		for (int i = -moveDistance; i <= moveDistance; i += moveDistance) { // William, you almost got it right. I just
																			// need to change two places in the code,
																			// i++ and j++
			for (int j = -moveDistance; j <= moveDistance; j += moveDistance) { // to i += moveDistance and j +=
																				// moveDistance ! Plus corresponding
																				// code in SimulationApp
				int newCol = col + i;
				int newRow = row + j;
				...
				if ((isCellValid(newCol, newRow) == TRUE)
					...
					/*
					 * if (isPlayerInCheck(pieceColor, board) == TRUE && ((pieceColor == WHITE &&
					 * row != 0) || (pieceColor == BLACK && row != 9)))
					 * continue;
					 */
	 ...
	 *         gotten a 1-piece of theirs to the given
	 *         player’s starting side of the board, and returns the column that
	 *         1-piece is in. Only
	 *         moves that capture this 1-piece will be valid,
	 *         and failure to capture it will result in a
	 *         checkmate.
	...
	public int isMoveValid(String fromCell, String toCell, int myColor, String[][] board) {
		...
		if (pieceMoveDistance == 0 || isMyPiece(fromCell, myColor, board) != TRUE
				|| isMyPiece(toCell, myColor, board) == TRUE)
			return FALSE;

		if ((Math.abs(cellToRow(fromCell) - cellToRow(toCell)) != 0
